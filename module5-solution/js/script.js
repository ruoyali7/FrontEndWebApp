...
dc.loadAboutPage = function () {
  ...
  $ajaxUtils.sendGetRequest(
    ...
    function (aboutHtml) {
      ...
      for (var i = filledStars + 1; i <= 5; i++) {
        aboutHtml = insertProperty(aboutHtml, "class" + i, "fa fa-star-o");
      }

      // Add the rating number next to stars
      aboutHtml = aboutHtml.replace('<span id="rating-text"></span>', `<span> ${rating}-star rating</span>`);

      insertHtml("#main-content", aboutHtml);

      document.querySelectorAll("#nav-list li").forEach(function (navItem) {
